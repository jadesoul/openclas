 ...
 *
 *	$Id: utility.hpp 93M 2012-06-24 01:12:16Z (local) $
 */
...
namespace openclas {
	...
	*/
	...
	const std::locale make_locale(enum Charset charset)
	{
		...
		// cout<<"now in const std::locale make_locale(enum Charset charset)"<<endl;
		try {
#if defined(_MSC_VER)
	...
	const std::locale locale_utf8=make_locale(CHARSET_UTF8);
	const std::locale locale_gbk=make_locale(CHARSET_GBK);
...
#	define locale_platform locale_utf8
	...
	* @returns true if it is exist in the collection.
	*/
