 ...
 *
 *	$Id: segment.hpp 93M 2012-06-24 01:12:16Z (local) $
 */
...
namespace openclas {
	...
	struct WordInformation {
		...
		WordInformation()
			: tag(WORD_TAG_UNKNOWN), weight(0), offset(0), length(0), 
			is_recorded(false), index(0), entry(0)
		{}
	...
	public:
		static std::wstring segment_to_string(const std::wstring& text, segment_type& seg, bool with_tag=true)
		{
			...
			for(std::vector<WordInformation>::iterator iSeg = seg.words.begin(); iSeg != seg.words.end(); ++iSeg)
			{
				...
				if (iSeg->tag != WORD_TAG_BEGIN && iSeg->tag != WORD_TAG_END) {
					...
					oss << word ;
					if (with_tag) oss << "/" << WORD_TAG_NAME[iSeg->tag];
					empty = false;
				}
			}
			return oss.str();
